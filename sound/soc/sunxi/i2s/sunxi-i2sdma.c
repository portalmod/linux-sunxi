...
//	printk("[I2S-DMA]Entered %s. \n", __func__);
    ...
    if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
    {
 //   	printk("[I2S-DMA]PLAYBACK POINTER.\n");
   		spin_lock(&prtd->lock);
   		sunxi_dma_getcurposition(prtd->params, (dma_addr_t*)&play_dmasrc, (dma_addr_t*)&play_dmadst);
		res = play_dmasrc + prtd->dma_period - prtd->dma_start;
		spin_unlock(&prtd->lock);
		if (res >= snd_pcm_lib_buffer_bytes(substream))		// TODO: Optimize conditional.
		{
    ...
    {
//    	printk("[I2S-DMA]CAPTURE POINTER.\n");
    	spin_lock(&prtd->lock);
